...
export default function StepTestClient({
  ...
  useEffect(() => {
    const loadPassage = async () => {
      ...
      // 아래쪽(더 쉬운 지문)은 -0.4까지만 허용한다. 한때 -0.7까지 넓혔었는데, 실측해보니
      // 2.0/3.0/4.0 선택 시 후보의 상당수(2.0은 절반)가 실제로는 그보다 한참 쉬운(center가
      // 선택 레벨보다 0.6 이상 낮은) 지문이라 "2.0을 선택했는데 체감 1.0 지문이 나오는" 반대
      // 방향 문제를 낳았다. -0.4로 좁히면 전 레벨에서 그런 사례가 0건이 된다(후보 수는 다소
      // 줄지만 반복 회피 로직이 돌아갈 만큼은 남음).
      const UPPER_RANGE = 0.3;
      const LOWER_RANGE = 0.4;
              ...
              <div className="space-y-6 px-6 py-6 sm:px-10">
                ...
                {finalResult?.endingDrops?.length > 0 && (
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      단어는 맞게 읽었지만 끝소리(과거형 -ed, 복수·3인칭 -s)를 흘렸습니다.
                    </p>
                    <div className="mt-3 flex flex-wrap gap-2">
                      {finalResult.endingDrops.slice(0, 5).map((w: string, i: number) => (
                        <span
                          key={i}
