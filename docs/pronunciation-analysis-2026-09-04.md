...
### 원문 텍스트를 오디오와 함께 준다 (중요)

우리는 Azure에 **아이 녹음(오디오) + 지문 원문 텍스트를 함께** 보낸다.

```
아이 녹음(오디오)  +  지문 원문 텍스트
        ↓  둘 다 함께 Azure 발음평가 API로
Azure: "이 오디오가 이 텍스트를 얼마나 잘 읽은 건지 채점"
```

```js
// 클라이언트(StepTestClient.tsx): 오디오 blob + ref( = passage.content) 둘 다 전송
form.append("audio", audioBlob);
form.append("text", refText);

// 서버(route.ts): 받은 text를 Azure 설정에 넣음
new PronunciationAssessmentConfig(text /* = 지문 원문 */, ...)
```

**Azure는 원문을 "정답지"로 삼는다.** 즉 "이 아이는 지금 이 문장을 읽는 중"이라는 걸
알고 듣는다. 그래서:

1. **인식을 원문 쪽으로 유도** — 애매하게 들려도 원문 단어로 받아쓴다.
2. **원문 단어마다 발음 점수**를 매긴다.
3. **들은 것을 원문에 정렬**해 생략/삽입/오발음을 표시한다.

이게 바로 **오류가 단어 레벨에서 안 보이는 이유**다:

| 아이가 실제로 읽은 것 | Azure 전사본 | 왜 |
|---|---|---|
| "walk" (어미 뺌) | **"walked"** | 원문이 walked라 어미를 복원 |
| "a" → "the"로 바꿔 읽음 | **"the"** | 원문이 the라 관사를 정규화 |
| "its" 건너뜀 | **"its"** (채워짐) | 원문에 its가 있어야 하니 언어모델로 채움 |
| "warm milk" 앞에 "large" 끼움 | "large" **버려짐** | 원문에 없는 단어라 인식이 약함 |

정규화가 안 된 경우(전사본에 실제로 다른 단어가 찍힌 경우)만 우리가 잡을 수 있다.

> **"없는 건 무시한다"의 정확한 뜻**: Azure는 삽입어를 *감지는 한다*
> (`ErrorType: "Insertion"`). 무시하는 건 **우리 코드** 쪽이다 — 삽입어를 (1) 발음 점수
> 계산에서 빼고(지문에 없는 단어가 "지문 읽기" 점수를 왜곡하면 안 되니까) (2) 화면에
> 표시하지 않는다(ASR이 삽입어를 자주 놓쳐 신뢰 불가).
...
**뿌리 원인**: 1절에서 설명했듯 원문을 정답지로 함께 주기 때문에, Azure가 아이의
근사한 발음을 원문 쪽으로 **정규화·복원**한다. 그 결과 아래 오류들은 전사본에
흔적이 안 남아 단어 레벨로는 감지 불가:

1. **관사 스왑 `the ↔ a`** — 약한 관사를 원문 쪽으로 정규화. (ASR이 우연히 다르게
   전사한 경우만 잡힘)
2. **기능어 스킵** (`its`, `of`, `the`) — 언어모델로 채워넣음.
3. **삽입어** — 원문에 없는 단어를 버림.
4. **어미 누락** — 음소 점수 편차로 녹음마다 ~4/5만 감지.

이런 오류들도 낮은 음소 점수를 통해 **발음 정확도 점수**에는 간접적으로 반영된다
(단어 목록에는 안 떠도 점수는 내려감).

> 이론적 대안: 원문을 안 주고 순수 STT(dictation)를 별도로 한 번 더 돌리면 아이가
> 실제로 뭐라고 했는지 더 날것으로 볼 수 있다. 단, 아이 영어 낭독에 대한 순수 STT는
> 정확도가 크게 떨어지고 Azure 호출 비용이 2배가 된다. 현재는 채택 안 함.
